...
describe("ClariFi API", () => {
  ...
  it("persists client notes and blocks client access to advisor audit data", async () => {
    ...
    await agent
      ...
      .send({ clientNotes: "Income while recovering matters to me." })
      .expect(200);
  ...
  it("extracts an uploaded PDF and returns page-level evidence", async () => {
    ...
    page.drawText(
      "Hospitalisation policy covers eligible inpatient hospital bills.",
      { x: 50, y: 720, size: 14, font },
    );
    page.drawText("Loss of income is not covered by this hospital plan.", {
      x: 50,
      y: 690,
      size: 14,
      font,
    ...
    const upload = await agent
      ...
      .attach("policy", Buffer.from(bytes), {
        filename: "demo-policy.pdf",
    ...
    const download = await agent
      .get(
        `/api/policies/${sessionId}/documents/${upload.body.document.id}/download`,
      )
      .expect(200);
  ...
  it("persists a generated client response and learning points", async () => {
        ...
        history: [
          {
            id: "question-1",
            role: "user",
            content: "Will this replace my income if I cannot work?",
          },
        ],
    ...
    const saved = await agent.get(`/api/sessions/${sessionId}`).expect(200);
    expect(saved.body.session.state.clientMessages.at(-1).role).toBe(
      "assistant",
    );
  ...
  it("lets advisors refine the illustrative product catalog and blocks clients", async () => {
    const catalog = Object.fromEntries(
      ["life", "investment", "critical", "shield", "retirement"].map(
        (category) => [
          category,
          [1, 2, 3].map((index) => ({
            name:
              category === "life" && index === 3
                ? "LifeGuard Plus"
                : `${category} product ${index}`,
            intent: `${category} intent ${index}`,
          })),
        ],
      ),
    );

    const advisor = request.agent(app);
    await advisor
      .post("/api/auth/demo-login")
      .send({ accountId: "advisor-demo" })
      .expect(200);
    const refined = await advisor
      .post("/api/products/refine")
      .send({
        instruction: "Replace LifeGuard Plus with FamilyGuard Term",
        catalog,
      })
      .expect(200);
    expect(refined.body.catalog.life[2].name).toBe("FamilyGuard Term");
    ...
    await client
      .post("/api/products/refine")
      .send({ instruction: "Replace a product", catalog })
      .expect(403);
  });
});
