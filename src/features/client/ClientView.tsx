...
export function ClientView(props: ClientViewProps) {
  ...
  const appendTranscript = (text: string) => {
    ...
    if (!trimmed) return;
    const line = `[${speakerRef.current}] ${trimmed}`;
    ...
    <>
      ...
      <div className="apple-panel-quiet p-3 shadow-none">
        ...
        <div className="mb-2 flex flex-wrap gap-2">
          <div className="flex rounded-lg border border-[#E5E5EA] bg-white/48 p-0.5 shadow-[inset_0_1px_0_rgba(255,255,255,.7)] backdrop-blur-xl">
            {(["Client", "Advisor"] as const).map((item) => (
              <button
                key={item}
                className={`rounded-md px-2.5 py-1 text-[11px] font-bold transition ${
                  speaker === item ? "bg-white/90 text-sci shadow-[0_8px_24px_rgba(0,0,0,.08)]" : "text-[#6E6E73] hover:text-ink"
                }`}
        ...
        <textarea
          ...
          placeholder="[Advisor] Hospitalisation cover reimburses eligible inpatient bills..."
