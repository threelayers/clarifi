...
import { z } from "zod";
import { demoAdvisorReply, demoClientReply, demoPreMeetingPrep, demoRecap } from "../demo.js";
import { demoFallbackEnabled, isProduction } from "../config.js";
import { requireAuth, requireRole } from "../middleware/auth.js";
import { aiLimiter } from "../middleware/rateLimit.js";
import { generateAdvisorReply, generateClientReply, generatePreMeetingPrep, generateSessionRecap } from "../openai.js";
...
const messagesFromHistory = (history: Array<z.infer<typeof messageSchema>>): SessionMessage[] =>
  history.map((message) => ({
...
aiRouter.post("/chat/client", async (req, res) => {
  ...
  const evidence = body.sessionId
    ? await getAppStore().searchPolicy(req.user!, body.sessionId, body.history.at(-1)?.content || "")
    : [];
  const context = {
    ...body,
    policyEvidence: evidence.map((item) => `${item.fileName}, page ${item.pageNumber}: ${item.quote}`).join("\n")
  };
  const reply = apiKey
    ...
    : demoFallbackEnabled
      ...
      : null;
  if (!reply) return res.status(503).json({ error: "Missing server OpenAI API key" });
  ...
  if (body.sessionId) {
    ...
    const nextMessages = latest && !history.some((message) => message.id === latest.id)
      ? [...history, latest, assistantMessage]
      : [...history, assistantMessage];
    await getAppStore().patchSession(req.user!, body.sessionId, {
...
aiRouter.post("/chat/advisor", requireRole("advisor"), async (req, res) => {
  ...
  const reply = apiKey
    ? await generateAdvisorReply(apiKey, body.history, body.clientTranscript, body.model, context)
    : demoFallbackEnabled
  ...
  if (body.sessionId) {
    ...
    await getAppStore().patchSession(req.user!, body.sessionId, {
      advisorMessages: [
        ...history,
        ...(latest && !history.some((message) => message.id === latest.id) ? [latest] : []),
        {
          id: crypto.randomUUID(),
          role: "assistant",
          text: reply.reply,
...
aiRouter.post("/recap", requireRole("advisor"), async (req, res) => {
  ...
  const recap = apiKey
    ...
    : demoFallbackEnabled
      ...
      : null;
  if (!recap) return res.status(503).json({ error: "Missing server OpenAI API key" });
  if (body.sessionId) {
    await getAppStore().patchSession(req.user!, body.sessionId, { recap, recapApproved: false }, "ai.recap.generated");
  }
  res.json(recap);
});
...
aiRouter.post("/premeeting", requireRole("advisor"), async (req, res) => {
  ...
  const prep = apiKey
    ...
    : demoFallbackEnabled
      ? demoPreMeetingPrep()
      : null;
  if (!prep) return res.status(503).json({ error: "Missing server OpenAI API key" });
  if (body.sessionId) {
    await getAppStore().patchSession(req.user!, body.sessionId, { preMeetingPrep: prep }, "ai.premeeting.generated");
  }
  res.json(prep);
});
